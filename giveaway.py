...
last_sent_timer = 0  # When to send another message to chat. Giveaway item, time left, chance of winning (number
# of entries)
claimTimer = 60
giveawayStarted = 0
was_drawn = 0
...
usage = "/me {user} -> !giveaway [start|stop|usage|draw] [Whats being give away]|[Duration is minutes]|[Access " \
        "Level]|[Max Entries]|[Point Cost] "


def run_timer(socket):
    global last_sent_timer, winner, isDrawn, hasClaimed, was_drawn, giveawayRunning, timeLeft, stopTimer, \
        giveawayQueue, enteredQueue, giveEntries
    if time.time() - last_sent_timer >= 60:
        last_sent_timer = time.time()
        if timeLeft == -1:
            utility.chat(socket, message)
        elif timeLeft != 0:
            utility.chat(socket, message + ". Type !enter to join. Giveaway ends in %s minutes. Current entries %s." % (
                timeLeft, len(giveawayQueue)))
            timeLeft -= 1
    ...
    if isDrawn is False and (time.time() - giveawayStarted >= duration):
        ...
        if len(giveawayQueue) > 0:
            ...
            utility.chat(socket, utility.command_formatter_message_without_points(
                "/me @{user} you have won. Speak up in the next 60 seconds or be rerolled", winner))
        else:
    ...
    if hasClaimed:
        ...
        utility.chat(socket, utility.command_formatter_message_without_points(
            "/me {user} has successfully claimed the prize. Win chance was %.2f%%." % winChance, winner))
...
def giveaway(sock, args, user):
    global giveawayRunning, winner, isDrawn, hasClaimed, pointCost, maxEntries, message, duration, giveawayStarted, \
        access_level, timeLeft, was_drawn, giveawayQueue, giveEntries, enteredQueue
    del args[0]
    doWhat = args[0]
    ...
    # Ends giveaway or without claiming
    elif doWhat == "stop":
        if giveawayRunning:
...
def enter(sock, user, args):
    ...
    if config.FOLLOW_REQ is True:
        print("stopped here")
        if follow is False or (time.time() - utility.convert_enddate_to_seconds(follow)) < 600:
            return
    if level < access_level:  # this fucks up 0 < 0 == true ... what?
        print("access level %s %s" % (access_level, level))
        # utility.chat(sock,utility.command_formatter_message_without_points("Sorry {user} your rank is too low to enter
        # the giveaway.",user))
        return
    ...
    if (points < pointCost * numOfEntries) and pointCost != 0:  # this fucks up
        ...
        # utility.chat(sock,utility.command_formatter_message_without_points("Sorry {user}, you don't have enough points
        # to enter the giveaway.",user))
        return
    if (user in giveEntries.keys()) is False:
