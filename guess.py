...
def timer(sock):
    ...
    if(time.time() - guessStarted >= guessTime and canEnter==True):
        canEnter=False
        utility.chat(sock,"/me Guessing has stopped. You cannot enter anymore.")
    if(time.time() - last_sent_timer >= 60 and guessingGiveaway==True and canEnter==True):
...
def claimTimer(socket):
    global guessBlue,guessRed,canEnter,guessRunning,guessStarted,guessingGiveaway,message,timeLeft,guessBlueG,guessRedG,winnersG,canCheck,isDrawn,was_drawn,hasClaimed,is_drawn,winnerG
    ...
    if(is_drawn==False):
        ...
        if(len(winnersG) >0):
            ...
            utility.chat(socket,utility.command_formatter_message_without_points("/me @{user} you have won. Speak up in the next 60 seconds or be rerolled",winnerG))
        else:
    ...
    if(hasClaimed):
        utility.chat(socket,utility.command_formatter_message_without_points("/me {user} has successfully claimed the prize.",winnerG))
        guessingGiveaway = False
        isDrawn = False
        hasClaimed = False
        canCheck=False
...
def guessStartEnd(sock,args,user):
    global guessBlue,guessRed,canEnter,guessRunning,guessStarted,guessingGiveaway,message,timeLeft,guessBlueG,guessRedG,winnersG,canCheck,isDrawn,was_drawn,guessingEntries
    del args[0]
    case = args[0]
    if(case == "start"):
        if(len(args)==1):
            ...
            guessBlue = {}
            guessRed = {}
            utility.chat(sock,"/me Guessing has started. Type !guess <teamcolor> <# kills> or !guess <teamcolor1> <# kills> <teamcolor2> <# kills> without <> to enter.")
        else:
            guessRunning=True
    ...
    elif(case=="end" and guessRunning==True and guessingGiveaway==True):
        ...
        guessBlueG={}
        timeLeft=-1
    elif(case == "end" and guessRunning == True and guessingGiveaway==False):
...
def guess(sock,user,args):
    ...
    number2=0
    win_loose=""
    if(len(args)==4 and guessingGiveaway==False):
    ...
    elif(len(args)==2 and guessingGiveaway==False):
        print("has successfully entered normal guessing")
        try:
            team1 = str(args[0])
            number1 = int(args[1])
        ...
        if(team1.lower() == "blue"):
            ...
            guessRed[user]=number1
    elif(len(args)==2 and guessingGiveaway==True):
